...
def init_widgets_list():
	...
	widget.Spacer(length=8,background=colors["red"],),
			...
			*([powerline_separator_left(colors["green"], colors["blue"], powerline_enabled)] if powerline_enabled else []),
			VolumeWidget(),
			*([powerline_separator_left(colors["gray"], colors["green"], powerline_enabled)] if powerline_enabled else []),
			nerd_dictation(),
			*([powerline_separator_left(colors["red"], colors["gray"], powerline_enabled)] if powerline_enabled else []),
			widget.GenPollText(
				name="updates",
				update_interval=30,
			...
			*([powerline_separator_left(colors["cyan"], colors["green"], powerline_enabled)] if powerline_enabled else []),
			ssh_widget,
		...
		*(
			...
			*([powerline_separator_left(colors["gray"], colors["cyan"], powerline_enabled)] if powerline_enabled else []),
			current_user_widget,
...
keys = [
	...
	# -b and -S will not work until i find a fix to allow a interval to be set, could hard code interval here
	KeyChord([alt], "w", [
	    Key([], "s",
	        lazy.spawn("/usr/local/bin/OpenFlexOS_WallpaperChanger.sh -s"),
	        lazy.ungrab_chord(),
	        desc="Select a static wallpaper"
	    ),
	    Key([], "r",
	        lazy.spawn("/usr/local/bin/OpenFlexOS_WallpaperChanger.sh -r"),
	        lazy.ungrab_chord(),
	        desc="Select a random wallpaper"
	    ),
	    Key([], "b",
	        lazy.spawn("/usr/local/bin/OpenFlexOS_WallpaperChanger.sh -b"),
	        lazy.ungrab_chord(),
	        desc="Start a wallpaper cycle"
	    ),
	    Key([], "e",
	        lazy.spawn("/usr/local/bin/OpenFlexOS_WallpaperChanger.sh -e"),
	        lazy.ungrab_chord(),

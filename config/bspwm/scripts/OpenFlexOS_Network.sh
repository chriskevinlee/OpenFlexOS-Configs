...
status_network(){
    ...
    if [[ $current_state != "$previous_state" ]]; then
        case "$current_state" in
            "$disconnected_icon") dunstify -u critical "Disconnected" ;;
...
wifi_network(){
    ...
    if [[ $main_menu = "󱚽 Connect to a Wifi Network" ]]; then
        ...
        forget_ssid=$(echo "$saved_wifi_connections" | $launcher -p "$wifi_forget")
...
while getopts "rdh" main 2>/dev/null; do
    case "$main" in
        ...
        d)
            ...
            ;;
        h)
              echo "A script to manage wifi network and to display current network status"
              echo "Usage: $(basename "$0") [ARGUMENT]"
              echo ""

              printf "%-30s %s\n" " -r" "Use Rofi to manage wifi network"
              printf "%-30s %s\n" " -d" "Use Dmenu to manage wifi network"
            ;;
        *)
